...
@pytest.fixture(scope='function')
def fixture_role_route_config_success(request):
    """
    Input:
    Output:
    Test Cases:
...
@pytest.fixture(scope='function')
def fixture_role_route_config_failure(request):
    """
    Input:
    Output:
    Test Cases:
...
class TestRRBAC():
    @pytest.mark.usefixtures("fixture_role_route_config_success")
    def test_role_route_config_success(
        self, fixture_role_route_config_success
    ):
        for index, data in enumerate(fixture_role_route_config_success):
    ...
    def test_role_route_config_failure(
        self, fixture_role_route_config_failure
    ):
        for index, data in enumerate(fixture_role_route_config_failure):
            print '\nScenario {} Started'.format(index + 1)
            with app.test_request_context(
                data['input']['url_rule'],
                method=data['input']['method']
